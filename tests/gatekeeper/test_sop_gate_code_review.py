...
class TestSOPGateCodeReview(unittest.TestCase):
    def setUp(self):
        self.orchestrator = Path.home() / ".gemini/antigravity/skills/Orchestrator/scripts/check_protocol_compliance.py"
        self.code_review_script = Path.home() / ".gemini/antigravity/skills/code-review/scripts/code_review.py"
    ...
    def test_finalization_blocks_without_code_review(self):
        ...
        result = subprocess.run(
            [sys.executable, str(self.orchestrator), "--status"],
            capture_output=True,
            text=True
        )
        self.assertIn("Code Review", result.stdout, "Orchestrator should mention Code Review status")
